...
import {BsLink, BsList} from "react-icons/bs";
import Timer from "@/app/components/Timer/Timer";

type MainBarProps = {
    probPage?: boolean
}
const MainBar: React.FC<MainBarProps> = ({probPage}) => {
                ...
                </Link>
                {
                    probPage && (
                        <div className={"flex items-center gap-1 flex-auto justify-center"}>
                            <div
                                className={"flex items-center justify-center rounded bg-dark-layer-1 hover:bg-blue-800 h-8 w-8 cursor-pointer"}>
                                <FaChevronLeft/>
                            </div>
                            <Link href={"/"}
                                  className={"flex items-center gap-2 font-medium max-2-[170px] text-white cursor-pointer"}>
                                <div>
                                    <BsList/>
                                </div>
                                Problems
